...
public interface MeterReadingRepository extends JpaRepository<MeterReading, Long> {
	...
	@Query("SELECT new com.example.waterrefillapijava.dto.projection.MeterReadingValue(m.readingDate, m.meterValue) FROM MeterReading m WHERE m.deleted = false ORDER BY m.readingDate DESC")
	List<MeterReadingValue> findReadingValuesByDeletedFalse();

	@Query("SELECT new com.example.waterrefillapijava.dto.projection.MeterReadingValue(m.readingDate, m.meterValue) FROM MeterReading m WHERE m.readingDate < :date AND m.deleted = false ORDER BY m.readingDate DESC")
