package com.example.waterrefillapijava.order.repository;
...
import com.example.waterrefillapijava.order.dto.BoardOrderResponse;
...
import com.example.waterrefillapijava.order.dto.projection.OrderItemProjection;
import com.example.waterrefillapijava.order.dto.projection.PaymentMixAggregate;
...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	@Query(
		"""
		SELECT new com.example.waterrefillapijava.order.dto.BoardOrderResponse(
			o.id,
			c.id,
		...
		ORDER BY CASE o.status
		  WHEN com.example.waterrefillapijava.order.model.OrderStatus.queued THEN 1
		  WHEN com.example.waterrefillapijava.order.model.OrderStatus.processing THEN 2
		  WHEN com.example.waterrefillapijava.order.model.OrderStatus.transit THEN 3
		  WHEN com.example.waterrefillapijava.order.model.OrderStatus.completed THEN 4
		END, o.id DESC
	""")
	...
	@Query(
		"""
		SELECT new com.example.waterrefillapijava.order.dto.projection.DailySalesAggregate(
			CAST(o.createdAt AS date),
			COUNT(o),
			COALESCE(SUM(o.totalAmount), 0),
			COALESCE(SUM(CASE WHEN o.paymentMethod = com.example.waterrefillapijava.order.model.PaymentMethod.cash THEN o.totalAmount ELSE 0 END), 0),
			COALESCE(SUM(CASE WHEN o.paymentMethod = com.example.waterrefillapijava.order.model.PaymentMethod.e_wallet THEN o.totalAmount ELSE 0 END), 0),
			COALESCE(SUM(CASE WHEN o.paymentMethod = com.example.waterrefillapijava.order.model.PaymentMethod.credit THEN o.totalAmount ELSE 0 END), 0)
		)
		FROM Order o
		WHERE o.status = com.example.waterrefillapijava.order.model.OrderStatus.completed AND o.deleted = false
	...
	@Query(
		"""
		SELECT new com.example.waterrefillapijava.meterreading.dto.projection.GallonsByDate(
			CAST(o.createdAt AS date),
			COALESCE(SUM(p.volumeGallons * oi.quantity), 0)
		)
		FROM Order o JOIN o.items oi JOIN oi.product p
		WHERE o.status = com.example.waterrefillapijava.order.model.OrderStatus.completed
			AND o.deleted = false
			AND p.type = com.example.waterrefillapijava.product.model.ProductType.water_refill
	...
	@Query(
		"""
		SELECT new com.example.waterrefillapijava.order.dto.projection.ProductSalesAggregate(
			p.id,
			p.name,
	...
	@Query(
		"""
		SELECT new com.example.waterrefillapijava.order.dto.projection.PaymentMixAggregate(
	...
	@Query(
		"""
		SELECT new com.example.waterrefillapijava.order.dto.projection.OrderItemProjection(
			oi.order.id,
			p.id,
			p.name,
			oi.quantity,
